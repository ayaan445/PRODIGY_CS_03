...
def check_password_strength(password):
    ...
    if strength == 1:
        remarks = ('Very weak password.'
                ' You may want to change it immediately.')
    elif strength == 2:
        ...
        remarks = ('Very strong password'
                ' Your account is secure than ever')
...
label = tk.Label(frame, text="Enter the password:", bg="lightgrey", fg="black", font=("Georgia", 14))
label.grid(row=0, column=0, padx=5, pady=5, sticky="e")

password_entry = tk.Entry(frame, show="*", font=("Georgia", 12))
password_entry.grid(row=0, column=1, padx=5, pady=5, sticky="we")

check_button = tk.Button(frame, text="Check", command=check_password, bg="green", fg="white", font=("Georgia", 12))
check_button.grid(row=1, column=0, pady=10, padx=5, sticky="we")

generate_button = tk.Button(frame, text="Generate Password", command=generate_password, bg="black", fg="white",
                            font=("Georgia", 12))
generate_button.grid(row=1, column=1, pady=10, padx=5, sticky="we")

copy_button = tk.Button(frame, text="Copy Password", command=copy_password, bg="lightgrey", fg="black",
                        font=("Georgia", 12))
copy_button.grid(row=1, column=2, pady=10, padx=5, sticky="we")

clear_button = tk.Button(frame, text="Clear", command=clear_input, bg="darkred", fg="white", font=("Georgia", 12))
clear_button.grid(row=1, column=3, pady=10, padx=5, sticky="we")

output_text = tk.Text(frame, height=10, width=60, state='disabled', font=("Georgia", 10))
